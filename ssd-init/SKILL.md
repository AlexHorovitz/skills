...
**Why `ssd/` (visible) rather than `.ssd/` (hidden):** visibility lets humans navigate the working docs in their file browser and IDE. Gitignore keeps them out of commits. If a specific artifact (e.g., an ADR) needs to be committed, move it to `docs/decisions/` explicitly — the SSD skills will not do that automatically.
...
- **1.0.0** (2026-04-18) — Initial implementation of first-run housekeeping workflow. Creates `ssd/`
  ...
  The earlier improvements proposal used `.ssd/` (hidden); this skill adopts the user's choice of
  `ssd/` (visible + gitignored).
