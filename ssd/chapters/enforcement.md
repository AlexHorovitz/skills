...
| `issue-sync-current` | [ADR-0014](../../docs/decisions/ADR-0014-github-issue-state-tracking.md) | (v2.4.0+) Informational GitHub mirror check. FAILs only on hard drift: a recorded issue closed while its workstream is active, or a `ssd:phase/*` label disagreeing with local phase. SKIPs by default (tracking off, no `gh`, or no issue binding); checks bindings before any network call. |
